...
public class TokenPopupMenu extends AbstractTokenPopupMenu {
	...
	if (MapTool.getFrame().getInitiativePanel()
		.hasOwnerPermission(tokenUnderMouse))
	...
	addGMItem(createChangeToMenu(Zone.Layer.GM, Zone.Layer.OBJECT,
		Zone.Layer.BACKGROUND));

	add(new JSeparator());

	/*
	 * This adds the expose menu to token right click when the player is GM
	 * and the server setting is set to use individual FOW
	 */
	...
	addToggledItem(new ShowPathsAction(),
		renderer.isPathShowing(tokenUnderMouse));
	addToggledItem(new SnapToGridAction(tokenUnderMouse.isSnapToGrid(),
		renderer), tokenUnderMouse.isSnapToGrid());
	...
	if (selectedTokenSet.size() != 1
		|| getTokenUnderMouse().getMacroNames(true).size() == 0) {
	    return null;
	}

	JMenu macroMenu = new JMenu("Macros");
	List<MacroButtonProperties> macroList = getTokenUnderMouse()
		.getMacroList(true);
	String group = "";
	...
	if (selectedTokenSet.size() != 1
		|| getTokenUnderMouse().getSpeechNames().size() == 0) {
	    return null;
	}

	JMenu menu = new JMenu("Speech");
	List<String> keyList = new ArrayList<String>(getTokenUnderMouse()
		.getSpeechNames());
	...
	if (getRenderer().getZone().getTokens() != null
		&& getRenderer().getZone().getTokens().size() != 0) {
	...
	public AddTokensExposedAreaAction(GUID theTokId) {
	    ...
	    String tokensView = I18N.getText(
		    "token.popup.menu.fow.tokens.view", sourceToken.getName());
	...
	public void actionPerformed(ActionEvent e) {
		...
		ExposedAreaMetaData targetMeta = targetToken
			.getExposedAreaMetaData();
		targetMeta.addToExposedAreaHistory((Area) sourceMeta
			.getExposedAreaHistory().clone());
		
		targetMeta=null;

		getRenderer().flush(targetToken);
		getRenderer().getZone().putToken(targetToken);
		MapTool.serverCommand().putToken(getRenderer().getZone().getId(), targetToken);
		targetToken=null;
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (Token tokenSource : allToks) {
		    ExposedAreaMetaData sourceMeta = tokenSource
			    .getExposedAreaMetaData();
		    for (GUID tok : selectedTokenSet) {
			Token token = getRenderer().getZone().getToken(tok);
			ExposedAreaMetaData meta = token
				.getExposedAreaMetaData();
			meta.addToExposedAreaHistory((Area) sourceMeta
				.getExposedAreaHistory().clone());
			meta = null;
			getRenderer().flush(token);
			getRenderer().getZone().putToken(token);
			MapTool.serverCommand().putToken(
				getRenderer().getZone().getId(), token);
			token = null;
		    }
		}
	    getRenderer().repaint();
	...
	public ExposeVisibleAreaOnlyAction() {
	    ...
	    // "Player visible area only (Ctrl - Shift - O)");
	    I18N.setAction("token.popup.menu.expose.currentonly", this, true);
	}

	public void actionPerformed(ActionEvent e) {
	    FogUtil.exposePCArea(getRenderer());
	    MapTool.serverCommand().exposePCArea(
		    getRenderer().getZone().getId());
	    getRenderer().repaint();
	}
	...
	return createColorAreaMenu("token.popup.menu.halo",
		getTokenUnderMouse().getHaloColor(), SetHaloAction.class,
		SetColorChooserAction.class);
    }

    private JMenu createColorAreaMenu(String title, Color selectedColor,
	    Class standardColorActionClass, Class customColorActionClass) {
	JMenu haloMenu = new JMenu(I18N.getText(title));

	try {
	    Constructor standardColorActionConstructor = standardColorActionClass
		    .getConstructor(new Class[] { TokenPopupMenu.class,
			    ZoneRenderer.class, Set.class, Color.class,
			    String.class });

	    Constructor customColorActionConstructor = customColorActionClass
		    .getConstructor(new Class[] { TokenPopupMenu.class,
			    ZoneRenderer.class, Set.class, String.class });

	    JCheckBoxMenuItem noneMenu = new JCheckBoxMenuItem(
		    (Action) standardColorActionConstructor
			    .newInstance(new Object[] { this, getRenderer(),
				    selectedTokenSet, null, "None" }));

	    JCheckBoxMenuItem customMenu = new JCheckBoxMenuItem(
		    (Action) customColorActionConstructor
			    .newInstance(new Object[] { this, getRenderer(),
				    selectedTokenSet, "Custom" }));
	...
	List<BarTokenOverlay> overlays = new ArrayList<BarTokenOverlay>(MapTool
		.getCampaign().getTokenBarsMap().values());
	if (overlays.isEmpty()) {
	    return null;
	}
	...
	// Create the group menus first so that they can be placed at the top of
	// the state menu
	Map<String, JMenu> groups = new TreeMap<String, JMenu>();
	...
	boolean isOwner = MapTool.getFrame().getInitiativePanel()
		.hasOwnerPermission(getTokenUnderMouse());
	if (isOwner) {
	    initiativeMenu
		    .add(new ChangeInitiativeState("initiative.menu.add"));
	    initiativeMenu.add(new ChangeInitiativeState(
		    "initiative.menu.remove"));
	    initiativeMenu.addSeparator();
	} // endif
	initiativeMenu.add(new JMenuItem(new ChangeInitiativeState(
		"initiative.menu.resume")));
	initiativeMenu.add(new JMenuItem(new ChangeInitiativeState(
		"initiative.menu.hold")));
	initiativeMenu.addSeparator();
	initiativeMenu.add(new JMenuItem(new ChangeInitiativeState(
		"initiative.menu.setState")));
	initiativeMenu.add(new JMenuItem(new ChangeInitiativeState(
		"initiative.menu.clearState")));

	// Enable by state if only one token selected.
	if (selectedTokenSet.size() == 1) {
	    List<Integer> list = MapTool.getFrame().getInitiativePanel()
		    .getList().indexOf(getTokenUnderMouse());
	    int index = list.isEmpty() ? -1 : list.get(0).intValue();
	    if (index >= 0) {
		if (isOwner)
		    initiativeMenu.getMenuComponent(0).setEnabled(false);
		boolean hold = MapTool.getFrame().getInitiativePanel()
			.getList().getTokenInitiative(index).isHolding();
		if (hold) {
		    initiativeMenu.getMenuComponent(isOwner ? 4 : 1)
			    .setEnabled(false);
		} else {
		    initiativeMenu.getMenuComponent(isOwner ? 3 : 0)
			    .setEnabled(false);
		}
	    } else {
		if (isOwner)
		    initiativeMenu.getMenuComponent(1).setEnabled(false);
		initiativeMenu.getMenuComponent(isOwner ? 4 : 3).setEnabled(
			false);
		initiativeMenu.getMenuComponent(isOwner ? 3 : 0).setEnabled(
			false);
	    } // endif
	} // endif
	...
	int percent = (int) (TokenBarFunction.getBigDecimalValue(value)
		.doubleValue() * 100);
	...
	public SetHaloAction(ZoneRenderer renderer, Set<GUID> tokenSet,
	...
	public SetColorChooserAction(ZoneRenderer renderer, Set<GUID> tokenSet,
		String name) {
	    this.tokenSet = tokenSet;
	    this.renderer = renderer;
	    this.currentColor = renderer.getZone()
		    .getToken(tokenSet.iterator().next()).getHaloColor();
	    putValue(Action.NAME, name);
	}
	...
	protected Color showColorChooserDialog() {
	    return JColorChooser.showDialog(
		    MapTool.getFrame().getContentPane(), "Choose Halo Color",
		    currentColor);
	}
    ...
    private class SetVisionOverlayColorChooserAction extends
	    SetColorChooserAction {
	public SetVisionOverlayColorChooserAction(ZoneRenderer renderer,
		Set<GUID> tokenSet, String name) {
	    super(renderer, tokenSet, name);
	    this.currentColor = renderer.getZone()
		    .getToken(tokenSet.iterator().next())
	...
	protected Color showColorChooserDialog() {
	    return JColorChooser.showDialog(
		    MapTool.getFrame().getContentPane(),
		    "Choose Vision Overlay Color", currentColor);
	}
	...
	public SetVisionOverlayColorAction(ZoneRenderer renderer, Set tokenSet,
		Color color, String name) {
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    JPanel labelPanel = new JPanel(new FormLayout("pref",
		    "pref 2px:grow pref"));
	    labelPanel.add(new JLabel(name + ":"), new CellConstraints(1, 1,
		...
		public void stateChanged(ChangeEvent e) {
		    JSlider js = (JSlider) ((JCheckBox) e.getSource())
			    .getClientProperty("JSlider");
		    js.setEnabled(!((JCheckBox) e.getSource()).isSelected());
		}
	    });
	    labelPanel.add(hide, new CellConstraints(1, 3,
		    CellConstraints.RIGHT, CellConstraints.TOP));
		...
		slider.setValue((int) (TokenBarFunction.getBigDecimalValue(
			getTokenUnderMouse().getState(name)).doubleValue() * 100));
	    }
	    JPanel barPanel = new JPanel(new FormLayout("right:pref 2px pref",
		    "pref"));
	    barPanel.add(labelPanel, new CellConstraints(1, 1));
	    barPanel.add(slider, new CellConstraints(3, 1));
	    if (JOptionPane.showOptionDialog(MapTool.getFrame(), barPanel,
		    "Set " + name + " Value", JOptionPane.OK_CANCEL_OPTION,
		    JOptionPane.PLAIN_MESSAGE, null, null, null) == JOptionPane.OK_OPTION) {
		Zone zone = MapTool.getFrame().getCurrentZoneRenderer()
			.getZone();
		for (GUID tokenGUID : selectedTokenSet) {
		    Token token = zone.getToken(tokenGUID);
		    BigDecimal val = hide.isSelected() ? null : new BigDecimal(
	...
	public void actionPerformed(ActionEvent aE) {
		...
		if (aE.getActionCommand().equals("clear")) {
		    for (String state : MapTool.getCampaign()
			    .getTokenStatesMap().keySet())
			token.setState(state, null);
		} else {
		    token.setState(
			    aE.getActionCommand(),
			    ((JCheckBoxMenuItem) aE.getSource()).isSelected() ? Boolean.TRUE
				    : null);
		} // endif
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    InitiativeList init = MapTool.getFrame().getInitiativePanel()
		    .getList();
	    String input = null;
	    if (name.equals("initiative.menu.setState")) {
		input = JOptionPane.showInputDialog(I18N
			.getText("initiative.menu.enterState"));
		if (input == null)
		    return;
		input = input.trim();
	    } // endif
		...
		if (name.equals("initiative.menu.add")
			|| name.equals("initiative.menu.addToInitiative")) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		getRenderer().showPath(token,
			!getRenderer().isPathShowing(getTokenUnderMouse()));
	    }
	    getRenderer().repaint();
	}
    }
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (path.getCellPath().get(0) instanceof CellPoint) {
		    zp = zone.getGrid().convert(
			    (CellPoint) path.getCellPath().get(0));
		} else {
	...
	public void actionPerformed(ActionEvent e) {
	    ...
	    JTextComponent commandArea = MapTool.getFrame().getCommandPanel()
		    .getCommandTextArea();
